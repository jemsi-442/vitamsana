...
const ProductModal = () => {
  ...
  const text = {
    en: {
      helper: 'Designed for a stronger restaurant feel: bold photography, cleaner pricing, and direct cart actions that support quick ordering.',
      added: 'Added to Cart!',
      add: 'Add to Cart',
    },
    sw: {
      helper: 'Imeundwa kwa muonekano bora wa restaurant: picha nzuri, bei zilizo wazi, na hatua za moja kwa moja za kuongeza kwenye kikapu kwa oda za haraka.',
  ...
  const localizedProduct = getLocalizedProduct(currentProduct, language);
        ...
        <div className="grid grid-cols-1 gap-6 overflow-y-auto p-6 md:grid-cols-2 md:p-8">
          ...
          <div className="relative">
            ...
            <p className="mb-4 text-2xl font-bold text-brand-600 dark:text-brand-200">
              {formatTsh(localizedProduct.price)}
            ...
            >
              {addedToCart ? text.added : text.add}
            </button>
          </div>
        </div>
      </div>
